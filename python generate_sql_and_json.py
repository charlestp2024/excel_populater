...
def main():
    ...
    for full_name, user_uuid in user_map.items():
        ...
        prosecution_sql.append(
            f"INSERT INTO `user` (`uuid`, `fullname`, `tenant_id`, `email`, `firstname`, `lastname`, `deleted`, `inactive`, `created_on`, `modified_on`) VALUES ('{user_uuid}', '{escape_sql(full_name)}', '{TENANT_ID}', '{email}', '{escape_sql(first_name)}', '{escape_sql(last_name)}', 0, 0, '{NOW_TIMESTAMP}', '{NOW_TIMESTAMP}');"
        )
    ...
    for client_name, data in client_hierarchy.items():
        ...
        prosecution_sql.append(
            f"INSERT INTO `client` (`id`, `client_name`, `client_code`, `tenant_id`, `email`, `is_active`, `created_on`, `modified_on`) VALUES ('{client_uuid}', '{escape_sql(client_name)}', {client_code_counter}, '{TENANT_ID}', '{client_email}', 1, '{NOW_TIMESTAMP}', '{NOW_TIMESTAMP}');"
        )
